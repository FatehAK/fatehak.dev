...
description: >-
  Learn how to automate your package publishing to NPM without any hassle using
  release-it
tags:
  - tooling
---
...
I've just created my first NPM package and was scouring the web for ways to automate the publishing step.
...
The whole process only took **around a minute** with **release-it**. Usually, we have to type in various commands in sequence for bumping the version, generating a git tag, creating a tarball, writing a changelog, and finally publishing the package to NPM.
...
The added support for lifecycle `hooks` is my favorite feature since it allows us to run tasks at various stages of publishing. Using these hooks, we could run our `lint` and `test` jobs before we begin the release process saving us time if they were to fail later.
